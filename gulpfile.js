...
gulp.task('optimize:html', ['optimize:css', 'optimize:js'], () => {
    console.log('Optimizing HTML files');
    let styleHash = /style.([0-9a-f]+).css/.exec(glob.sync(path.join(dist, 'css/style.*.css')));
    let depsHash = /deps.([0-9a-f]+).js/.exec(glob.sync(path.join(dist, 'js/deps.*.js')));
    let s2aHash = /s2a.([0-9a-f]+).js/.exec(glob.sync(path.join(dist, 'js/s2a.*.js')));
    ...
    return gulp.src('**/*.html', {cwd: src})
        ...
        .pipe(replace(
            ...
            `<link rel="stylesheet" href="/css/style.${styleHash}.css">`),
        )
        .pipe(replace(
            '<script src="/js/deps.js">',
            `<script src="/js/deps.${depsHash}.js">`),
        )
        .pipe(replace(
            '<script src="/js/s2a.js">',
            `<script src="/js/s2a.${s2aHash}.js">`),
        )
        .pipe(gulp.dest(dist));
});
...
gulp.task('lint', ['lint:thumbnails']);

gulp.task('optimize-for-deployment', [
    'copy:static',
    'optimize:html',
    'optimize:css',
    'optimize:js',
